...
export default class ForestGenerator {
     ...
     * @param {object} config
     */
    constructor(biomesOperator, config) {
        let _this = this;
    ...
    cutTrees(forestMap, filledCells) {
        ...
        filledCells
            .slice(0, filledCells.length * this.config.FOREST_DIE_CHANCE)
    ...
    expandTrees(forestMap, filledCells, step) {
        ...
        let _this = this,
            ...
            chance = step <= _this.config.STEPS_BOOST_STEPS ? _this.config.FOREST_GROWTH_CHANCE * 3 : _this.config.FOREST_GROWTH_CHANCE,
    ...
    createTrees(forestMap, filledCells, step) {
        ...
        for (i = 0; i < potentialCells.length; i++) {
            ...
            createChance = _this.getCreateChance(
                ...
                x,
                y,
                step <= _this.config.STEPS_BOOST_STEPS
                    ? _this.config.FOREST_BORN_CHANCE * _this.config.FOREST_BORN_BOOST
                    : _this.config.FOREST_BORN_CHANCE
            );
    ...
    getCreateChance(forestMap, humidity, x, y, speed) {
        ...
        let waterRatio = Math.max(1, this.config.FOREST_CREATE_MULTS.WATTER / biome.getDistanceToWater()),
            humidityRatio = changeRange(humidity, 0, 1, 0, this.config.FOREST_CREATE_MULTS.HUMIDITY),
