...
export default class RiversMap extends BinaryMatrix {
    constructor(altitudeMap, lakesMap) {
        ...
        this.generateMap = function () {
            let riverSources = this.getRiverSources(this.altitudeMap), rivers = this.generateRiversCells(riverSources);
        ...
        this.generateRiversCells = function (riverSources) {
            let _this = this, rivers = [], allRiversPoints = [], riversLimit = Math.floor(fromFraction(Config.RIVERS_DENSITY, 1, Config.WORLD_SIZE)), startCloseness = Math.max(Config.RIVER_MIN_LENGTH, Config.RIVER_START_CLOSENESS);
            for (let i = 0; i < riverSources.length; i++) {
                ...
                for (let j = 0; j < Config.WORLD_SIZE; j++) {
                    ...
                    let x = nextRiverPoint[0], y = nextRiverPoint[1], altitude = _this.altitudeMap.getCell(x, y);
        ...
        this.getRiverDirection = function (river, altitudeMap) {
            let currentPoint = river[river.length - 1], prevPoint = river.length > 1 ? river[river.length - 2] : false, cx = currentPoint[0], cy = currentPoint[1], currentAltitude = altitudeMap.getCell(cx, cy), neighbors = altitudeMap.getNeighbors(cx, cy).shuffle(), lowerPoint = null;
            for (let i = 0; i < neighbors.length; i++) {
                let nx = neighbors[i][0], ny = neighbors[i][1], altitude = altitudeMap.getCell(nx, ny);
        ...
        this.addRiverDeltaToRiverMap = function (river) {
            let _this = this, ratio = randBetweenNumbers(0.01, Config.RIVER_DELTA_MAX_LENGTH), deltaLength = river.length * ratio, notDeltaLength = river.length - deltaLength, delta = [];
