...
export default class RiversMap extends BinaryMatrix {
    ...
    generateRiversCells = function(riverSources) {
        ...
        let _this = this,
            rivers = [],
            allRiversPoints = [],
            riversLimit = Math.floor(fromFraction(_this.config.RIVERS_DENSITY, 1, _this.config.WORLD_SIZE)),
            startCloseness = Math.max(_this.config.RIVER_MIN_LENGTH, _this.config.RIVER_START_CLOSENESS);
        ...
        for (let i = 0; i < riverSources.length; i++) {
            ...
            for (let j = 0; j < _this.config.WORLD_SIZE; j++) {
                ...
                if (_this.altitudeMap.isWater(altitude) || arrayHasPoint(allRiversPoints, x, y)) {
                    ...
                    if (_this.lakesMap.filled(x, y)) {
                        ...
                        if (lakeSize > river.length * _this.config.LAKE_TO_RIVER_RATIO) {
                            finished = true;
                            break;
                        }
