...
export default class IslandsMap extends NumericMatrix {
   ...
   */
  private getIsland(startX: number, startY: number): Array<[number, number]> {
    const islandCells: Array<[number, number]> = [];
    ...
    while (stack.length > 0) {
      ...
      notOceanMap.foreachFilledAround(x, y, (nx: number, ny: number) => {
        const key = `${nx},${ny}`;
        if (!visited.has(key)) {
          visited.add(key);
          stack.push([nx, ny]);
